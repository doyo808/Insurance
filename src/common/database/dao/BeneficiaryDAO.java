...
public class BeneficiaryDAO {
    ...
    public int insert(BeneficiaryModel model, Connection conn) throws SQLException {
        String query = "INSERT INTO BENEFICIARIES (beneficiary_id, beneficiary_name, relationship, bank_account, bank_name) " +
                       "VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement pstmt = conn.prepareStatement(query)) {
            ...
            pstmt.setString(5, model.getBank_name());
            return pstmt.executeUpdate();
        }
    }

    public int update(BeneficiaryModel model, Connection conn) throws SQLException {
        String query = "UPDATE BENEFICIARIES SET beneficiary_name = ?, relationship = ?, bank_account = ?, bank_name = ? " +
                       "WHERE beneficiary_id = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(query)) {
            pstmt.setString(1, model.getBeneficiary_name());
            pstmt.setString(2, model.getRelationship());
            pstmt.setString(3, model.getBank_account());
            pstmt.setString(4, model.getBank_name());
            pstmt.setInt(5, model.getBeneficiary_id());
            return pstmt.executeUpdate();
        }
    }
