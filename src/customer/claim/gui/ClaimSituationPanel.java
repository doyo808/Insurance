	...
	public ClaimSituationPanel(JPanel parentCardPanel) {
		this.parentCardPanel = parentCardPanel;
	      ...
	      // 상단 제목
	       ...
	       String[] 다친신체부위 = {"머리(얼굴포함)", "어깨/목", "허리", "팔", "손/손목", "다리", "무릎", "발/발목", "해당되는 신체부위가 없어요"};
	       String[] 부상유형선택 = {"상처가 났어요(손상)", "접질렀어요(염좌)", "부러졌어요(골절)", "데였어요(화상)", "해당되는 부상종류가 없어요"};
	       
	       // 병원-상해
	       // 교통사고에서도 - 어디서 발생했는지 이후 적는칸 있음, 
	       String 다친상황; // 고객이 직접 입력하는 JTextField 활용예정 (ex 계단에서 넘어져 발목을 다쳤어요, 놀이기구에서 떨어져 타박상을 입었어요, 목욕탕에서 미끄러져 허리를 다쳤어요
...
//	       String 다친상황; // 고객이 직접 입력하는 JTextField 활용예정 (ex 보행 중 지나가는 오토바이에 치였어요, 운전 중 앞차량과 부딪혀 손목을 다쳤어요, 버스가 급정거하면서 넘어졌어요)
	        ...
	        */
	       JComboBoxMaker 상황콤보박스 = new JComboBoxMaker(상황);
	       ...
	       부상유형선택콤보박스.addActionListener((e) -> {
	    	    if (!부상유형선택콤보박스.isVisible()) return;
