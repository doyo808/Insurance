...
public class AutoPaymentPanel2 extends BasicPanel {
	...
	public AutoPaymentPanel2() {
		...
		setLayout(new MigLayout("", "[200px,grow][1040px,grow][200px]", "[][][404.00,grow][]"));
		...
		PaymentDefaultButton checkButton = new PaymentDefaultButton("확인");
		checkButton.setAlignmentX(Component.CENTER_ALIGNMENT);
		checkButton.addActionListener(e -> {
		    String[] selected = ((ContractTablePanel) ContractPanel).getSelectedContract();
		    if (selected != null) {
		        System.out.println("선택된 계약: " + Arrays.toString(selected));
		    } else {
		        System.out.println("선택된 계약이 없습니다.");
		    }
		});
		add(checkButton, "cell 1 3,alignx center"); // MigLayout 위치 조정
	}
	...
	public static void main(String[] args) {
		...
		f.setBounds(0, 0, 1440, 1024);
		

		f.getContentPane().add(new AutoPaymentPanel2());
